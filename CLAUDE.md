...
- Equity orders go through `POST /trade/place` in one call, with the `orders` row id as `client_order_id` (D-018): market, day. Whole `units` by default; for accounts the user ticked as `fractional`, a dollar amount as `notional_value` with `units: null` (D-016): all the cash on a buy, the exact remaining dollars on a sell, no buffer, no floor. Never decimal `units` (Wealthsimple refuses them) and never the legacy `/trade/impact` + `/trade/{tradeId}` pair (it does not work with SnapTrade's order webhooks).
