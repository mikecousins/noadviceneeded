 ...
 * happened. Legs run one at a time; a failure in one never stops the others,
 * except a missing trade scope, which would fail them all the same way.
 */
export async function executeBatch(
  ...
  for (const leg of input.legs) {
    ...
    try {
      ...
      await db
        .update(orders)
        .set({ snaptradeTradeId: impact.trade.id, status: "checked" })
        .where(eq(orders.id, row.id));
