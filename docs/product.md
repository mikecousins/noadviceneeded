...
6. **Invest.** When cash is in any included account, the Invest page shows what each account can buy at the current price (whole units with 1% held back for market moves, or every cent as a dollar amount for accounts the user marked as fractional) and one button places a market day order in each account through SnapTrade. The Orders page shows what happened.
...
8. **Withdraw.** The user enters an amount. The Withdraw page walks accounts in the withdrawal order, sells until the amount is covered (whole units, or the exact dollar amount where marked fractional), shows the tax note for each account type, and one button places the sells. The user then moves the cash out at the brokerage.
